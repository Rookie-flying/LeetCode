...
public class MinimumAbsoluteDifferenceInBst {
    ...
    class Solution {
        ...
        public void minSubtraction(TreeNode node) {

            if (node == null) {
                return;
            }

            minSubtraction(node.left);

            if (pre != null && Math.abs(node.val - pre.val) < min) {
                min = Math.abs(node.val - pre.val);
            }
            pre = node;

            minSubtraction(node.right);
        }
    }
