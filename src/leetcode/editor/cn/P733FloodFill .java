...
class P733FloodFill{
    public static void main(String[] args) {
        ...
        solution.floodFill(images, sr, sc, newColor);
    }
...
class Solution {
    ...
    public void DFS(int[][] images, int i, int j, int newColor, int target) {
        ...
        if (images[i][j] == target) {
            images[i][j] = newColor;
        }

        DFS(images, i, j+1, newColor, target);
        DFS(images, i, j-1, newColor, target);
        DFS(images, i+1, j, newColor, target);
        DFS(images, i-1, j, newColor, target);
    }
}
